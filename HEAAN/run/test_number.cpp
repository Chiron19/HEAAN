...
int main(int argc, char **argv) {
	...
	for (long i = 0; i < min(20L, numFloats); ++i) {
		cout << setprecision(4) << doubleArray[i] << " ";
	}
    cout << endl;
	...
	// Real Decrypt //
	complex<double>* dec0 = scheme.decrypt(secretKey, cipher2);

	cout << "First 20 decrypted values:" << endl;
	for (long i = 0; i < min(20L, numFloats); ++i) {
		cout << setprecision(5) << dec0[i].real() << " + 1j*" << dec0[i].imag() << " ";
	}
    cout << endl;
    ...
    for (long i = 0; i < min(10L, numFloats); ++i) {
		cout << setprecision(4) << dec1[i].real() << " + 1j*" << dec1[i].imag() << " ";
	}
    cout << endl;
	ring.EMB(dec1, slots);
	...
	cout << "First 20 decrypted values:" << endl;
	for (long i = 0; i < min(20L, numFloats); ++i) {
		cout << setprecision(4) << doubleArray_[i] << " ";
	}
    cout << endl;
